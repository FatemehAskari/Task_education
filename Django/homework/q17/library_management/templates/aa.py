...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-path', default='learngamma_MiniImageNet/save/5shot')
    ...
    for j in range(len(gamma_list)):
        ...
        filename = f'lr_effect_MiniImageNet/5shotCSVfile_train/5shot${gamma_list[j]}.csv'
        ...
        with open(filename, 'w', newline='') as file:
            ...
            for i in range (len(weights)):
                convolutional_network=resnet18(pretrained=True)
                convolutional_network.fc=nn.Flatten()
                model_true=PrototypicalNetworks(convolutional_network,weightlearnable=True,gammalearnable=True,weight=weights[i],gamma1=gamma_list[j],
                ...
                regular_params = [param for param in model_true.parameters() 
                                  if not any(param is 
                                             ...
                                             in [model_true.weight_1, model_true.weight_2
                                                 ...
                                                 , model_true.weight_5,
                                                 model_true.gamma1,model_true.gamma2])]
                ...
                optimizer = optim.Adam([
                    ...
                    {'params':[model_true.gamma1,
                                model_true.gamma2],'lr': 0.005}
                ], lr=0.0001)
                ...
                print(model_true.gamma1.item(),model_true.gamma2.item())
                ...
                result_after_false=evaluate(val_loader,model=model_true)
                torch.save(model_true.state_dict(), osp.join(save_path, f'model_false_${gamma_list[j]}_${i}' + '.pth'))                                                                                
                ...
                values = [weights[i][0],weights[i][1],weights[i][2],
                        ...
                        weight_after5_true,gamma_list[j],gamma_list[j],model_true.gamma1.item(),model_true.gamma2.item(),0.0001,
