...
class BookFilterTest(TestCase):
    ...
    def test_all_blank_params(self):
        ...
        self.assertEqual(expected, content, '\nدر صورتی که همه‌ی کوئری‌پارامترها خالی باشند؛ باید تمامی کتاب‌ها را بازگردانید.')
    ...
    def test_max_price_only(self):
        ...
        expected = render_to_readable_output(books)
        content = response.content.decode('utf-8')
        self.assertEqual(expected, content, '\nدر صورتی که فقط کوئری‌پارامتر max_price داخل url وجود داشته باشد؛ باید کتاب‌هایی که قیمت‌ آن‌ها کمتر از قیمت آمده شده است را بازگردانید.')